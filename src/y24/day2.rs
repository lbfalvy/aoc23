...
pub fn safe_bar_one(
  seq: &[i32],
  mut chk: impl FnMut(i32, i32) -> bool,
) -> bool {
...
pub fn day2() {
  let reports: Vec<Vec<i32>> = fetch_input_lines(2)
    .map(|l| l.split(' ').map(|s| s.trim().parse().unwrap()).collect_vec())
    .collect_vec();
  let safe = reports
    .iter()
    .filter(|v| {
      let deltas = v.iter().tuple_windows().map(|(a, b)| a - b).collect_vec();
      deltas.iter().all(|d| 0 < *d && *d < 4)
        || deltas.iter().all(|d| -4 < *d && *d < 0)
    })
    .count();
